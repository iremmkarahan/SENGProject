<?php
include 'partials/header.php';
include '../config/database.php'; // Ensure this file contains the database connection setup

// fetch all rooms from database
$query = "SELECT id, title, category_id FROM rooms ORDER BY id DESC";
$rooms = mysqli_query($connection, $query);

if (!$rooms) {
    die("Database query failed: " . mysqli_error($connection));
}
?>

<section class="dashboard">
    <?php if (isset($_SESSION['add-room-success'])) : ?>
        <div class="alert__message success container">
            <p>
                <?= $_SESSION['add-room-success'];
                unset($_SESSION['add-room-success']);
                ?>
            </p>
        </div>
    <?php elseif (isset($_SESSION['edit-room-success'])) : ?>
        <div class="alert__message success container">
            <p>
                <?= $_SESSION['edit-room-success'];
                unset($_SESSION['edit-room-success']);
                ?>
            </p>
        </div>
    <?php elseif (isset($_SESSION['edit-room'])) : ?>
        <div class="alert__message error container">
            <p>
                <?= $_SESSION['edit-room'];
                unset($_SESSION['edit-room']);
                ?>
            </p>
        </div>
    <?php elseif (isset($_SESSION['delete-room-success'])) : ?>
        <div class="alert__message success container">
            <p>
                <?= $_SESSION['delete-room-success'];
                unset($_SESSION['delete-room-success']);
                ?>
            </p>
        </div>
    <?php endif ?>
    <div class="container dashboard__container">
        <button id="show__sidebar-btn" class="sidebar__toggle"><i class="uil uil-angle-right-b"></i></button>
        <button id="hide__sidebar-btn" class="sidebar__toggle"><i class="uil uil-angle-left-b"></i></button>
        <aside>
            <ul>
                <li>
                    <a href="add-booking.php"><i class="uil uil-user-plus"></i>
                        <h5>Add Booking</h5>
                    </a>
                </li>
                <li>
                    <a href="manage-booking.php"><i class="uil uil-building"></i>
                        <h5>Manage Bookings</h5>
                    </a>
                </li>
                <?php if (isset($_SESSION['user_is_admin'])) : ?>
                    <li>
                        <a href="add-user.php"><i class="uil uil-user-plus"></i>
                            <h5>Add User</h5>
                        </a>
                    </li>
                    <li>
                        <a href="manage-users.php"><i class="uil uil-users-alt"></i>
                            <h5>Manage User</h5>
                        </a>
                    </li>
                    <li>
                        <a href="add-category.php"><i class="uil uil-edit"></i>
                            <h5>Add Category</h5>
                        </a>
                    </li>
                    <li>
                        <a href="manage-categories.php"><i class="uil uil-list-ul"></i>
                            <h5>Manage Categories</h5>
                        </a>
                    </li>
                    <li>
                        <a href="add-facility.php"><i class="uil uil-plus"></i>
                            <h5>Add Facility</h5>
                        </a>
                    </li>
                    <li>
                        <a href="manage-facilities.php"><i class="uil uil-building"></i>
                            <h5>Manage Facilities</h5>
                        </a>
                    </li>
                    <li>
                        <a href="add-room.php"><i class="uil uil-pen"></i>
                            <h5>Add Room</h5>
                        </a>
                    </li>
                    <li>
                        <a href="manage-rooms.php" class="active"><i class="uil uil-postcard"></i>
                            <h5>Manage Rooms</h5>
                        </a>
                    </li>
                <?php endif ?>
            </ul>
        </aside>
        <main>
            <h2>Manage Rooms</h2>
            <?php if (mysqli_num_rows($rooms) > 0) : ?>
                <table>
                    <thead>
                        <tr>
                            <th>Title</th>
                            <th>Category</th>
                            <th>Edit</th>
                            <th>Delete</th>
                        </tr>
                    </thead>
                    <tbody>
                        <?php while ($room = mysqli_fetch_assoc($rooms)) : ?>
                            <?php
                            $category_id = $room['category_id'];
                            $category_query = "SELECT title FROM categories WHERE id=$category_id";
                            $category_result = mysqli_query($connection, $category_query);
                            $category = mysqli_fetch_assoc($category_result);
                            ?>
                            <tr>
                                <td><?= $room['title'] ?></td>
                                <td><?= $category['title'] ?? 'Uncategorized' ?></td>
                                <td><a href="<?= ROOT_URL ?>admin/edit-room.php?id=<?= $room['id'] ?>" class="btn sm">Edit</a></td>
                                <td><a href="<?= ROOT_URL ?>admin/delete-room.php?id=<?= $room['id'] ?>" class="btn sm danger" onclick="return confirm('Are you sure you want to delete this room?')">Delete</a></td>
                            </tr>
                        <?php endwhile ?>
                    </tbody>
                </table>
            <?php else : ?>
                <div class="alert__message error"><?= "No rooms found" ?></div>
            <?php endif ?>
        </main>
    </div>
</section>

<?php
include '../partials/footer.php';
?>
